...
N_total=10000; % change N here; Generate number of samples from a 4-component GMM
alpha_true = [0.1,0.2,0.3,0.4];
mu_true = [5 -5 -5 5;-5 -5 5 5];
Sigma_true(:,:,1) = [2,-1;-1,1.5];
Sigma_true(:,:,2) = [5,4;4,4.5];
Sigma_true(:,:,3) = [2.5,1;1,2.5];
Sigma_true(:,:,4) = [2.5,0;0,2.5];%set four different mu and sigma
x = randGMM(N_total,alpha_true,mu_true,Sigma_true);
M=1; % M should be change here

logLikelihood2 = 0;
%repeat the experienment for 30 times 
for times=1:30
    logLikelihood2sum=0;
    for i=1:10
        N=N_total;
...
%             displayProgress(t,y,alpha,mu,Sigma);
            ...
            for k=1:(N/0.9)/10
                for l = 1:M 
                h=h + alphaNew(l,:)*evalGaussian(y2(:,k),muNew(:,l),SigmaNew(:,:,l));
                end
                summ=summ+log(h); 
            end
        end
        logLikelihood2sum = logLikelihood2sum + summ;
    end
    logLikelihood2 =logLikelihood2+ logLikelihood2sum/10;

end
logLikelihood = logLikelihood2 / 30,
